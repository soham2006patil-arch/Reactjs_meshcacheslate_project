...
export default function DataLocationMap({ servers, activeNodeId, setActiveNodeId, addMockData }) {
  ...
  return (
    <div className="bg-dashboard-card p-4 rounded-xl border border-dashboard-border flex flex-col h-auto">
      <h2 className="text-lg font-semibold text-slate-100 mb-4"> Simplified Data Location Map</h2>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
        {servers.map((server) => (
          <button
            ...
            className={`flex flex-col items-center justify-center p-3 rounded-lg border transition-all ${activeNodeId === server.id
      ...
      <div
        className={`transition-all duration-300 ease-in-out overflow-hidden ${activeServer
